...
function print_ast(sourceFile) {
    emit(sourceFile);
    function emitNodeArray(narr) {
        write("[");
        var first = true;
        for (var elt of narr) {
            if (!first)
    ...
    function emit(val) {
        ...
        if (val instanceof Array)
            return emitNodeArray(val);
        var node = val;
        emitArbitraryNode(node);
    }
    function emitArbitraryNode(node) {
    ...
    function getArbitraryNodeName(node) {
        switch (node.kind) {
            ...
            case ts.SyntaxKind.OfKeyword:
                return "OfKeyword"; // LastKeyword and LastToken
...
const sourceFile = ts.createSourceFile("dump.ts", "const i = {}; I.x = null", ts.ScriptTarget.Latest);
