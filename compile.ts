...
function generateDocumentation(fileNames: string[], options: ts.CompilerOptions): void 
{
    ...
    function compileProgram(node : ts.Program)
    {
    ...
    function compileSourceFile(node: ts.SourceFile) : any
    {
    ...
    function compileStatement(node: ts.Statement) 
    {
        switch (node.kind)
        {
            ...
            {
                const id : ts.InterfaceDeclaration = node as ts.InterfaceDeclaration;
    ...
    function compileSignature(node : ts.NamedDeclaration)
    {
        switch (node.kind)
        {
            case ts.SyntaxKind.PropertySignature:
                return compilePropertySignature(node as ts.PropertySignature);
            case ts.SyntaxKind.MethodSignature:
                return compileMethodSignature(node as ts.MethodSignature);
            case ts.SyntaxKind.CallSignature:
                return compileCallSignatureDeclaration(node as ts.CallSignatureDeclaration);
            default: throw new Error("cannot handle signature kind " + node.kind);
        }
        
    }

    function compileCallSignatureDeclaration(node: ts.CallSignatureDeclaration)
    {
        return node;
    }

    function compileMethodSignature(node: ts.MethodSignature)
    {
        return node;
    }

    function compilePropertySignature(node: ts.PropertySignature)
    {
        return node;
    }

    function compileType(node: ts.TypeNode)
    {
        return node;
    }
